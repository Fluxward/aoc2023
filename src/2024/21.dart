...
int _rab(List<P> a, List<P> b, int l) {
  ...
  while (!ListEquality<P>().equals(q.first.r, b)) {
    ...
    if (c.r.last == b.last) {
      int pt = 0; //rab(c.r, b, l - 1);
      if (l > 3) {
        pt = rab([dpA, ...c.r.take(2)], [dpA, ...b.take(2)], l - 1);
      } else if (l == 3) {
        pt = rab([...c.r.take(2)], [...b.take(2)], l - 1);
      } else {
        pt = rab([c.r.first], [b.first], 1);
      }
      q.add(St21(b, c.d + pt, c));
      continue;
    }
    ...
    for (dir d in dir.values) {
      ...
      cost += rab(
          c.r.sublist(0, np.length - 1), np.sublist(0, np.length - 1), l - 1);
      // press A
      cost++;
...
int tl = 6;
int complexity(List<int> li) => li.foldIndexed<int>(0, (i, p, e) {
      //print("Moving from ${i == 0 ? 'A' : li[i - 1]} to $li");
      int r = (rab(
          [dpA, dpA, i == 0 ? kpA : kpm[li[i - 1]]!], [dpA, dpA, kpm[e]!], tl));
      return p + 1 + r;
    });
void d21(bool sub) {
  ...
  print(ii.map(complexity).foldIndexed<int>(0, (i, p, e) {
    return p + e * int.parse(input[i].substring(0, input[i].length - 1));
  }));
  print(st.elapsed);
}
